...
def run_geometry_forge(input_path: str, output_path: str, args):
    ...
    with zipfile.ZipFile(output_path, 'w', compression=zipfile.ZIP_STORED) as holo_zip:
        with concurrent.futures.ProcessPoolExecutor(max_workers=args.workers) as executor:
            ...
            for tensor in tensor_iterator:
                ...
                # OOM FIX: Throttle submission to queue based on active workers
                if len(active_futures) >= args.workers * 2:
            ...
            # Drain the remaining futures in the queue
            if active_futures:
    ...
    holo_file_size = os.path.getsize(output_path)
    
    sparsity_rate = 100.0 * (1.0 - (total_surv / total_orig)) if total_orig > 0 else 0.0
    compression_rate = 100.0 * (1.0 - (holo_file_size / initial_file_size)) if initial_file_size > 0 else 0.0
    ...
    print(f"Qubed Survivors  : {total_surv:,}")
    print(f"Sparsity Rate    : {sparsity_rate:.2f}%")
    print("-" * 50)
    print(f"Input File Size  : {format_size(initial_file_size)}")
    print(f"Output .holo Size: {format_size(holo_file_size)}")
    print(f"Disk Compression : {compression_rate:.2f}% reduction")
    print("-" * 50)
